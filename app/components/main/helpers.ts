...
export const calculateVanDerWaalsForce = (
  distance: number,
  sigma: number,
  epsilon: number
) => {
...
export const calculateElectrostaticForce = (
  charge1: number,
  charge2: number,
  distance: number,
  dielectricConstant: number = 80
) => {
...
export const calculateHydrogenBondStrength = (
  distance: number,
  angle: number
) => {
...
export const generateDNAPath = () => {
  ...
  for (let i = 0; i < segments; i++) {
    ...
    const majorGrooveDepth =
      Math.sin(t * Math.PI * turns * 2) * majorGrooveWidth;
    const minorGrooveDepth =
      Math.sin(t * Math.PI * turns * 2 + Math.PI) * minorGrooveWidth;
  ...
  return Array(count)
    ...
    .map((_, i) => {
      const outDegree = Math.min(
        Math.floor(Math.random() * maxOutDegree) + 1,
        count - 1
      );
      return Array(outDegree)
        .fill(0)
        .map(() => ({
          source: {
          ...
          strength:
            Math.random() * (1 - minBindingAffinity) + minBindingAffinity,
    ...
    return Array(connectionCount)
      ...
      .map(() => {
        ...
        return {
          ...
          strength:
            intraClusterStrength *
            (1 + target.connectivity * preferentialAttachment),
        };
      });
  });
};
